...
async function loadTileset(tileset: EmbeddedTileset, mapUrl: string): Promise<LoadedTileset> {
	...
	const image = await loadImage(resolveAssetUrl(mapUrl, tileset.image));
	return {
		name: tileset.name,
...
function resolveAssetUrl(mapUrl: string, assetPath: string): string {
	const base = new URL(mapUrl, window.location.href);
