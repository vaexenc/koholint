...
import {validateFeedback} from "@/shared/protocol/feedback";
...
import type {FeedbackStore} from "../store/feedback";
...
export function createApi({feedback, adminToken}: ApiDeps) {
	// per-IP so one player can't bury the panel in submissions. the client is
	// served by a separate process that proxies this route, so the key comes
	// from the forwarded address rather than the (always-proxy) peer.
	...
	api.post("/feedback", async (c) => {
		...
		const sender = resolveClientIp(
			getConnInfo(c).remote.address,
			c.req.header("x-forwarded-for")
		);
		if (!submitLimiter.tryTake(sender))
