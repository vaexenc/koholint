import {SpriteCanvas} from "@/client/components/avatar/SpriteCanvas";
...
import {spriteCanvasSize} from "@/client/sprites";
import {NAME_MAX_LENGTH} from "@/shared/protocol/validateName";
import {CLASSIC_CHARACTER_ANIMATIONS} from "@/shared/sprites/animations";
import {AVATARS, resolveAvatar, type Avatar} from "@/shared/sprites/avatars";
import {PALETTES, resolvePaletteSwap, type NamedPalette} from "@/shared/sprites/palettes";
...
// overhangs its host's top-right corner — which is the badge's own business, not
// each call site's — so hosts must not clip it: the swatch rows never clip, and
// the avatar grid reserves top/right padding for it.
function SelectedBadge() {
	return (
		<span className="pointer-events-none absolute -top-1.5 -right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm ring-2 ring-background">
...
// the name field's whole contract, as one group. "name editing is on" is a
// single fact, so it is a single optional prop rather than four loose ones that
// have to agree — a name with no way to change it is now unrepresentable, and
// neither this component nor its host re-derives whether the field is shown.
export type NameFieldProps = {
	...
	// save).
	error?: string | null;
	// lets the owning surface focus the field from outside (e.g. to pull a
...
export type AvatarPickerProps = {
	...
	// present in online mode; omitted offline, where the picker is name-less.
...
export function AvatarPicker({
	...
	const selected = resolveAvatar(avatarId);
	...
	return (
		<>
			{nameField ? <NameField {...nameField} /> : null}
